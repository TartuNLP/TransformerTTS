...
class CNNResNorm(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.convolutions = [
            tf.keras.layers.Conv1D(filters=f, kernel_size=kernel_size, padding=padding)
            for f in filters[:-1]
        ]
        self.inner_activations = [
            tf.keras.layers.Activation(inner_activation)
            for _ in range(self.n_layers - 1)
        ]
        self.last_conv = tf.keras.layers.Conv1D(
            filters=filters[-1], kernel_size=kernel_size, padding=padding
        )
    ...
    def build(self, input_shape):
        super(CNNResNorm, self).build(input_shape)
...
class TransposedCNNResNorm(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        super(TransposedCNNResNorm, self).build(input_shape)
...
class FFNResNorm(tf.keras.layers.Layer):
    ...
    def __init__(
        self, model_dim: int, dense_hidden_units: int, dropout_rate: float, **kwargs
    ):
    ...
    def build(self, input_shape):
        super(FFNResNorm, self).build(input_shape)
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, v, k, q_in, mask=None, training=False, **_):
        ...
        concat_attention = tf.reshape(
            scaled_attention, (tf.shape(scaled_attention)[0], -1, self.model_dim)
    ...
    def call(self, inputs, training=False, **_):
        ...
        if mask is not None:
            scaled_attention_logits += (
                mask * -1e9
            )  # TODO: add mask expansion here and remove from create padding mask
...
class SelfAttentionResNorm(tf.keras.layers.Layer):
    ...
    def call(self, x, training=False, mask=None, **_):
        attn_out, attn_weights = self.mha(
            x, x, x, mask, training=training
        )  # (batch_size, input_seq_len, model_dim)
        return self.last_ln(attn_out + x), attn_weights

    def build(self, input_shape):
        super(SelfAttentionResNorm, self).build(input_shape)
...
class SelfAttentionDenseBlock(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.supports_masking = True
        self.sarn = SelfAttentionResNorm(
            model_dim, num_heads, dropout_rate=dropout_rate
        )
    ...
    def call(self, x, training=False, mask=None):
        ...
        dense_mask = 1.0 - tf.squeeze(mask, axis=(1, 2))[:, :, None]
        attn_out = attn_out * dense_mask
        return self.ffn(attn_out, training=training) * dense_mask, attn_weights

    def build(self, input_shape):
        super(SelfAttentionDenseBlock, self).build(input_shape)
...
class SelfAttentionConvBlock(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        if transposed_convs:
            self.conv = TransposedCNNResNorm(
...
class SelfAttentionBlocks(tf.keras.layers.Layer):
    def __init__(
        ...
        transposed_convs: bool | None = None,
        ...
        **kwargs,
        ...
        self.supports_masking = True
        self.model_dim = model_dim
        self.pos_encoding_scalar = tf.Variable(1.0)
    ...
    def call(self, inputs, training=False, mask=None, reduction_factor=1):
        ...
        x += (
            self.pos_encoding_scalar
            * self.pos_encoding[:, : seq_len * reduction_factor : reduction_factor, :]
        )
        x = self.dropout(x, training=training)
        attention_weights = {}
        for i, block in enumerate(self.encoder_SADB):
            x, attn_weights = block(x, training=training, mask=mask)
            attention_weights[f"{self.name}_DenseBlock{i + 1}_SelfAttention"] = (
                attn_weights
            )
        for i, block in enumerate(self.encoder_SACB):
            x, attn_weights = block(x, training=training, mask=mask)
            attention_weights[f"{self.name}_ConvBlock{i + 1}_SelfAttention"] = (
                attn_weights
            )
    ...
    def build(self, input_shape):
        super(SelfAttentionBlocks, self).build(input_shape)
...
class StatPredictor(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        super(StatPredictor, self).build(input_shape)
...
class CNNDropout(tf.keras.layers.Layer):
    def __init__(
        ...
        self.dropouts = [
            tf.keras.layers.Dropout(rate=dout_rate) for _ in range(self.n_layers)
        ]
        self.normalization = [
    ...
    def build(self, input_shape):
        super(CNNDropout, self).build(input_shape)
...
class Expand(tf.keras.layers.Layer):
    ...
    def call(self, x, dimensions=None, **_):
        ...
        index_masks = tf.RaggedTensor.from_row_lengths(
            tf.ones(tot_dim), tf.reshape(dimensions, [-1])
        ).to_tensor()
        index_masks = tf.cast(
            tf.reshape(index_masks, (batch_size, seq_len * max_dim)), tf.float32
        )
        non_zeros = seq_len * max_dim - tf.reduce_sum(max_dim - dimensions, axis=1)
        # stack and mask
        tiled = tf.tile(x, [1, 1, max_dim])
        reshaped = tf.reshape(
            tiled, (batch_size, seq_len * max_dim, self.model_dimension)
        )
        mask_reshape = tf.multiply(reshaped, index_masks[:, :, tf.newaxis])
        ragged = tf.RaggedTensor.from_row_lengths(
            mask_reshape[index_masks > 0], non_zeros
        )
